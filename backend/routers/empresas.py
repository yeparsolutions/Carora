...
async def invitar_colaborador(
    ...
    # Username único dentro de la misma empresa (dos empresas distintas pueden tener mismo username)
    existe = db.query(Usuario).filter(
        Usuario.username   == username,
        Usuario.empresa_id == usuario_actual.empresa_id
    ).first()
    if existe:
        raise HTTPException(status_code=409, detail=f"El usuario '@{username}' ya está en uso en esta empresa")
