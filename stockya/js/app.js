...
function actualizarUIUsuario() {
  ...
  // Cargar selector de sucursal si aplica
  cargarSelectorSucursal();
}

/* ============================================================
   SELECTOR DE SUCURSAL — sidebar (solo admin Pro con 2+ sucursales)
   ============================================================ */
var _sucursalActivaId = null;

async function cargarSelectorSucursal() {
  var wrap = document.getElementById("selectorSucursalWrap");
  var sel  = document.getElementById("selectorSucursal");
  if (!wrap || !sel) return;

  var rol = usuarioActual && usuarioActual.rol;
  if (rol !== "admin") { wrap.style.display = "none"; return; }
  ...
  try {
    ...
    sel.innerHTML = sucursales
      .filter(function(s) { return s.activa; })
      .map(function(s) {
        return "<option value='" + s.id + "'>" + _esc(s.nombre) + "</option>";
      }).join("");

    // Restaurar selección previa si existe
    if (_sucursalActivaId) sel.value = _sucursalActivaId;
    else _sucursalActivaId = parseInt(sel.value);

    // Listener cambio
    sel.onchange = function() {
      _sucursalActivaId = parseInt(this.value);
      // Recargar pantalla actual con la nueva sucursal
      var screens = ["dashboard","productos","stock","movimientos","salidas","reportes"];
      var activa  = document.querySelector(".screen.active");
      if (activa) {
        var name = activa.id.replace("screen-","");
